...
int sizeOfType(Type* type) {
  ...
  case TP_INT:
  case TP_CHAR:
    return 1;  // Each integer/char occupies 1 word (4 bytes)
  case TP_ARRAY:
...
void declareObject(Object* obj) {
  if (obj->kind == OBJ_PARAMETER) {
    ...
    if (owner->kind == OBJ_FUNCTION) {
      ...
      obj->paramAttrs->localOffset = RESERVED_WORDS + owner->funcAttrs->paramCount;
      owner->funcAttrs->paramCount++;
      // Update frameSize to point after the parameters
      symtab->currentScope->frameSize = RESERVED_WORDS + owner->funcAttrs->paramCount;
    } else if (owner->kind == OBJ_PROCEDURE) {
      addObject(&(owner->procAttrs->paramList), obj);
      obj->paramAttrs->localOffset = RESERVED_WORDS + owner->procAttrs->paramCount;
      owner->procAttrs->paramCount++;
      // Update frameSize to point after the parameters
      symtab->currentScope->frameSize = RESERVED_WORDS + owner->procAttrs->paramCount;
    }
  ...
  // Add object to current scope's object list or global list
